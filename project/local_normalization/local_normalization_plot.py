...
def volatility_one_stock_plot(dates: List[str], time_step: str) -> None:
    """Plots the volatility of one stock.
    ...
    function_name: str = volatility_one_stock_plot.__name__
    correlation_matrix_tools \
    ...
    try:
        ...
        # Load data
        volatility_data: pd.DataFrame = pickle.load(open(
                        f'../data/correlation_matrix/volatility_data'
                        + f'_{dates[0]}_{dates[1]}_step_{time_step}'
                        + f'.pickle', 'rb'))

        figure: plt.Figure = plt.figure()

        plot: np.ndarray = volatility_data.plot(figsize=(16, 9), grid=True)

        plt.title(f'Volatility one stock from {dates[0]} to {dates[1]}'
